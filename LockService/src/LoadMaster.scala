...
class LoadMaster (val numNodes: Int, val numGroup: Int, val servers: Seq[ActorRef], val burstSize: Int) extends Actor {
...
object LoadMaster {
   def props(numNodes: Int, numGroup: Int, servers: Seq[ActorRef], burstSize: Int): Props = {
      Props(classOf[LoadMaster], numNodes, numGroup, servers, burstSize)
   }
}
